package by.Gorbatsevich.L_30_01_2025;

public class Task00 {
    public static void main(String[] args) {
//        Wheel[] wheels = new Wheel[4];
//
//        for (int i = 0; i < wheels.length; i++) {
//            wheels[i] = new Wheel("Зимняя", 17, "Резина");
//        }

        //Car audi = new Car("Audi", "A6", 2006, wheels);
        //System.out.println(audi);
        WheelArray wheelArray = new WheelArray(4);
        wheelArray.add(new Wheel("Зимняя", 17, "Резина"));
        // wheelArray.add(new Wheel("Зимняя", 17, "Резина"));
        // wheelArray.add(new Wheel("Зимняя", 17, "Резина"));
        //  wheelArray.add(new Wheel("Зимняя", 17, "Резина"));
        //System.out.println(wheelArray);
        // wheelArray.add(new Wheel("Летняя", 16, "Резина"));
        //System.out.println(wheelArray);
        Wheel wheel = wheelArray.get(wheelArray.size - 1);
        System.out.println(wheel);
        wheelArray.sortByDiametr();
        System.out.println(wheelArray);
        wheelArray.sortByType();
        System.out.println(wheelArray);

        int size = wheelArray.getSize();
        // Wheel[] wheels = wheelArray.getSize();
    }
}
